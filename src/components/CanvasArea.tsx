...
export default function CanvasArea({ width, height, stageRef }: CanvasAreaProps) {
  ...
  const handleDragMove = (id: string, x: number, y: number, target?: any) => {
    ...
    let bestVertical: { delta: number; position: number; label?: string } | null = null;
    let bestHorizontal: { delta: number; position: number; label?: string } | null = null;
    ...
    if (bestVertical) {
      absX += bestVertical.delta;
      movingRect.x = absX;
      candidates.push({
        orientation: 'vertical',
        position: bestVertical.position,
      label: bestVertical.label,
      distance: Math.abs(bestVertical.delta),
      });
    }

    if (bestHorizontal) {
      absY += bestHorizontal.delta;
      movingRect.y = absY;
      candidates.push({
        orientation: 'horizontal',
        position: bestHorizontal.position,
        label: bestHorizontal.label ?? (movingTextLike ? 'baseline' : undefined),
        distance: Math.abs(bestHorizontal.delta),
      });
    }
